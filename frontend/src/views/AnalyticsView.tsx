      ...
      <div className="flex flex-col items-center justify-center py-16 gap-4 text-center max-w-md mx-auto">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            No analytics data could be found for slug{" "}
            <code className="font-mono font-bold">/{slug}</code>. The link may
            have expired or not exist yet.
          </p>
        </div>
    ...
    <div className="space-y-6 w-full">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Daily clicks, referrers, and locations for this link.
          </p>
        </div>
        ...
        <CardHeader className="pb-2 border-b">
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-xs">
                Aggregated daily engagement history
              </CardDescription>
            </div>
        ...
        <CardContent className="pt-4 h-[320px]">
          {analytics.clicksPerDay.length > 0 ? (
            <Line
              ...
              aria-label={`Daily clicks chart for /${slug}`}
            />
          ) : (
            <div className="flex items-center justify-center h-full text-xs text-muted-foreground">
              No click history recorded yet. Share your short link to start
              tracking!
            </div>
          )}
        </CardContent>
          ...
          <div className="px-6 pb-4">
            <details className="text-xs text-muted-foreground">
              <summary className="cursor-pointer hover:text-foreground font-medium">
                View daily clicks as a table (
                {analytics.totalClicks.toLocaleString()} total)
              </summary>
              <div className="mt-2 max-h-48 overflow-auto rounded-xl border">
                <table className="w-full font-mono text-[11px]">
                  ...
                  <tbody>
                    {analytics.clicksPerDay.map((d) => (
                      <tr key={d.date} className="border-t">
                        <td className="px-3 py-1">{d.date}</td>
                        <td className="px-3 py-1 text-right">{d.count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="rounded-2xl border bg-card/70 backdrop-blur">
          <CardHeader className="border-b pb-3">
            ...
            <CardDescription className="text-xs">
              Traffic origin and referring platforms
            </CardDescription>
          </CardHeader>
